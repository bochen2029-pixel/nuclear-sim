...
1. **M4-T3** (recommended — NEXT ACTION) — GPU eigen path (propagate the M4-T2-b fission bank through the power iteration) + differential validation vs ref; unblocked (M4-T2-b done). The formal `nukebench` G0c gate awaits M1-T5 (blocked).
...
| M4-T3 | todo | — | — | M1-T3, M4-T2-b | gpu eigen: drive `physics/eigen` via a GpuTransport wrapper, **propagate the M4-T2-b fission bank** (fission-source iteration). Differential vs ref now; FORMAL G0c gate needs `nukebench`+`gates.toml` (M1-T5, behind blocked M1-T4a-2) |
