...
**Convention note (C-040…C-043, MAJ-11):** C-041 derives from C-040 (180 MeV prompt-deposited) and is the ONLY constant converting fissions to yield (E6). C-042/C-043 are Sher & Beck total-energy-basis values, provided for cross-check readouts only; readouts using them MUST label the basis difference. That difference is **per nuclide, not one figure** (measured in `docs/VERIFICATION.md` §1, M0-T3-b): against the C-040 basis it is **+5.3% for C-042** (Pu-239, implied ≈190 MeV) but only **+0.5% for C-043** (U-235, implied ≈181 MeV), because Sher & Beck report a different release per nuclide. A blanket "~5%" label — as this note previously carried — overstates C-043's gap by an order of magnitude.
