...
| M0-T3 | Author `spec/appendix/constants.data.toml` (strict sibling, 03 §1: values+lo/hi from the human appendix; PENDING for unresolved) + `tools/gen_constants` → `data/constants.toml` + headers + **`docs/VERIFICATION.md`** (see below) | `tools/gen_constants/`, `tools/verify/`, `spec/appendix/`, `docs/` | generator fails on missing cite/status; `constants_roundtrip` bijection passes under ctest; **every species named in any material resolves to a molar mass (hard error otherwise, appendix §3 rule)**; headers compile; `VERIFICATION.md` regenerates byte-identically. VERIFY: `ctest -R constants` |
...
| M0-T6 | CI. (a) `tools/ci/local_ci.(ps1\|sh)`: configure+build+ctest for win-x64 + linux presets, non-zero on failure; GitHub Actions workflow committed. (b) **owner step (ADR-011: public repo + MIT):** owner creates the public GitHub repo; session pushes; Actions green on main | `tools/ci/`, `.github/workflows/` | (a) local CI exits 0. (b) blocked_on: owner creating repo; then CI green on main |
...
| M1-T2 | `ref/` history-based transport (E1a–E1e, implicit capture) + fixed-source | `src/ref/` | DoD per `05 §1`: pure-capturer point-source leakage exp(−Σ_c·R) within 1σ at 3 optical depths (Σ_f = Σ_s = 0 required) AND infinite-medium k_inf = ν̄Σ_f/(Σ_c+Σ_f) within 3σ |
