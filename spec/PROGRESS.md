...
- **NEXT ACTION:** Execute **M3-T1** — α-mode kinetics E3a–E3c (`src/physics/kinetics/`), driven by the eigen outputs `k_prompt`/Λ/β_eff (ADR-013). Runnable now (`depends_on = M1-T3`, done) and needs **no** `fast4` xs — unit-test on toy eigen outputs as M1-T3 did. It advances the M2/M3→G2 physics path on the CPU oracle while the xs blocker holds. See `spec/05-module-transport.md §3`, `spec/01-physics.md` (E3); DoD in `07-milestones.md`. **Why not more M4 perf:** M4-T4-b measured the event-based eigen swap to REGRESS 2–6× (per-superstep overhead × 30 generations); the real fix is the M4-T4-c cooperative-groups megakernel — a substantial GPU effort, **perf-only, and the eigen is not a current bottleneck** (no gate runs it at scale until `fast4`), so it is low priority vs physics progress. **Critical-path blocker (unchanged):** **M1-T4a-2 (owner cited xs data)** gates M1-T5 → the FORMAL gate reports (G0a/G0b, M4-T3-b G0c) and all of M2, and the remaining G4 criteria (so **C-945 stays PENDING**). Providing a citable multigroup library or an ENDF+collapse pipeline unblocks the whole downstream chain — highest-value if the owner can supply it.

## Ready-queue (runnable now)

1. **M3-T1** (recommended — NEXT ACTION) — α-kinetics E3a–E3c (`physics/kinetics`); `depends_on = M1-T3` (**done**), runnable now, needs no `fast4` xs (unit-testable on toy eigen outputs like M1-T3 was). Advances the M2/M3→G2 physics path on the CPU oracle while the xs blocker holds (downstream M3-T2/T3, G2 stay blocked).
2. **M4-T4-c** — cooperative-groups megakernel eigen (land M4-T4-b's divergence fix WITHOUT the per-superstep overhead that made M4-T4-b regress). Unblocked (M4-T4-a done). **Perf-only, low priority:** the eigen is not a current bottleneck (no gate runs it at scale until `fast4`), and it is a substantial GPU effort for uncertain gain.
