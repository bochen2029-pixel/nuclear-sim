...
- **NEXT ACTION:** Execute **M4-T2** — event-based transport kernels on the M4-T1 foundation: per-superstep tentative-event sampling → event tag → **prefix-sum event partition** into per-event queues → branchless per-event kernels, plus the **deterministic fission bank** that writes progeny at the exclusive-prefix-sum slots M4-T1's scan reserves (parent-identity streams via `rng::fork`, never buffer position). DoD (`07`): a T-diff subset on a fixed-source toy case (ref↔gpu statistical agreement, G0c criteria). Reuse `src/gpu/` (`buffers.cuh` SoA; `gpu_backend`'s scan + fixed-point reduction). Keep the CUDA guard so CI stays at 75. See `spec/05-module-transport.md §6`; DoD in `07-milestones.md`. (M4-T3 — GpuTransport eigen path + G0c harness — hard-depends on M1-T3 (done) but comes after M4-T2.)

## Ready-queue (runnable now)

1. **M4-T2** (recommended — NEXT ACTION) — event kernels + deterministic fission bank on the M4-T1 foundation; fully unblocked (M4-T1 done), parallel with M2/M3 (ADR-009).
2. **M4-T3** — GpuTransport eigen path + G0c harness; needs M4-T2 (and M1-T3, done).
