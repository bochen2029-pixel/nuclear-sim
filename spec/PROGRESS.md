...
- **Milestone:** M1 — CPU reference transport + bare-sphere benchmarks (M0 complete; M1-T1..T3 + T4a-1 done). **M4 in progress in parallel (ADR-009); SYNC-M4 run; M4-T1 + M4-T2-a + M4-T2-b + M4-T3-a done — the GPU event-based transport AND k-eigenvalue power iteration are live and differentially validated against the CPU oracle.**
- **VERIFY:** `cmake --preset win-x64 && cmake --build --preset win-x64-rel && ctest --preset win-x64-rel` — the `12 §2` canonical loop; falsifiable and real (configure + compile + link + tests). **On the dev machine (CUDA auto-on): 88 passing = 75 CPU + 13 `gpu.` (on the RTX 4070 Ti SUPER). On a CPU-only build / the CI runners (`NUKESIM_WITH_CUDA` OFF): 75.** Takes ~1 min warm. Per-task probes are **anchored** (`11 §1`): `ctest --preset win-x64-rel -R "^<module>\."` for module ∈ {toolchain, constants, oracle, rng, loaders, geometry, ref, eigen, benchmarks, gpu} — all resolve and are disjoint. `^gpu\.` selects 13 and only registers when `NUKESIM_WITH_CUDA` is ON. Disjoint **and** exhaustive still holds: the anchored counts sum to the total — 6+11+1+10+11+15+7+9+5+13 = 88 (or 75 on a CPU-only build).
- **NEXT ACTION:** Execute **M4-T4** — the GPU performance pass + G4. Fold in the per-event **branchless-kernel split** behind the prefix-sum partition (warp divergence) that M4-T2-b deferred, and swap the M4-T3-a eigen's history-per-thread transport for the event-based superstep transport. Profile with Nsight (FP64 instruction fraction ≤ 1%, `05 §6`/MAJ-33); record gen/s, VRAM (`--vram-report`) and the batch-vs-throughput curve to `artifacts/perf_history.jsonl`; resolve C-945 (G4 budgets, PENDING) by measurement; **G4 executed and findings recorded** (never "G4 passes"). See `spec/05-module-transport.md §6`, `spec/08-validation.md §G4`; DoD in `07-milestones.md`. **Critical-path blocker to surface to the owner:** **M1-T4a-2 (cited xs data) now gates most remaining non-perf work** — it blocks M1-T5 (`nukebench`/`gates.toml`), hence the FORMAL gate reports (G0a/G0b, and M4-T3-b's G0c), M1-T5 itself, and all of M2. Providing a citable multigroup library or an ENDF+collapse pipeline unblocks the whole downstream chain.

## Ready-queue (runnable now)

1. **M4-T4** (recommended — NEXT ACTION) — GPU perf pass + G4 (branchless-kernel split; event-based transport inside the eigen; Nsight / gen-s / VRAM; C-945 by measurement); unblocked (M4-T3-a done). One of the few tasks NOT gated by M1-T4a-2.
2. *(**Blocked on M1-T4a-2** (owner cited-data): M1-T5 nukebench/gates → the formal gate reports incl. **M4-T3-b** (G0c) and G0a/G0b; and all of M2. M1-T4a-2 is the critical path for non-perf work.)*
...
| M4-T4 | in_progress | session-2026-08-02-d | 2026-08-02 | M4-T3-a | perf pass + G4; includes the per-event **branchless-kernel split** (warp divergence) deferred by M4-T2-b, the event-based transport inside the eigen, and Nsight FP64-fraction ≤ 1% |
