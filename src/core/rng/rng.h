...
class Stream {
public:
    NUKESIM_HD constexpr Stream(std::uint64_t seed, std::uint64_t stream,
    ...
    NUKESIM_HD float normal_f() noexcept {
    ...
    NUKESIM_HD constexpr std::pair<std::uint64_t, std::uint8_t> state() const noexcept {
        return {ctr_, sub_};
    }

    NUKESIM_HD constexpr std::uint64_t stream_id() const noexcept { return stream_; }

private:
    NUKESIM_HD constexpr void refill() noexcept {
    ...
    NUKESIM_HD constexpr std::uint32_t next_u32() noexcept {
...
NUKESIM_HD constexpr std::uint64_t rotl64(std::uint64_t x, int k) noexcept {
    return (x << k) | (x >> (64 - k));
}
...
NUKESIM_HD constexpr std::uint64_t splitmix64(std::uint64_t x) noexcept {
...
NUKESIM_HD constexpr std::uint64_t fork(std::uint64_t parent_stream, std::uint64_t parent_ctr,
