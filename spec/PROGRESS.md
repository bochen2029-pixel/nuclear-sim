...
- **Milestone:** M1 — CPU reference transport + bare-sphere benchmarks (M0 complete; M1-T1..T3 + T4a-1 done). **M4 started in parallel (ADR-009); SYNC-M4 run; M4-T1 done — the GPU backend foundation (SoA buffers + device Philox + deterministic slots/streams/reductions) is live and green.**
- **VERIFY:** `cmake --preset win-x64 && cmake --build --preset win-x64-rel && ctest --preset win-x64-rel` — the `12 §2` canonical loop; falsifiable and real (configure + compile + link + tests). **On the dev machine (CUDA auto-on): 81 passing = 75 CPU + 6 `gpu.` (on the RTX 4070 Ti SUPER). On a CPU-only build / the CI runners (`NUKESIM_WITH_CUDA` OFF): 75.** Takes ~1 min warm. Per-task probes are **anchored** (`11 §1`): `ctest --preset win-x64-rel -R "^<module>\."` for module ∈ {toolchain, constants, oracle, rng, loaders, geometry, ref, eigen, benchmarks, gpu} — all resolve and are disjoint. `^gpu\.` selects 6 and only registers when `NUKESIM_WITH_CUDA` is ON. Disjoint **and** exhaustive still holds: the anchored counts sum to the total (75, or 81 with CUDA).
- **NEXT ACTION:** Execute **M4-T2-a** (SPLIT from M4-T2) — port the static data the GPU transport reads onto the device: a float `DeviceLayerStack` + `__host__ __device__` analytic ray-sphere tracker (`distance_to_boundary` / `locate` / nudge-along-direction, mirroring `core/geometry` in **float** per `01 §9`), and a compact device material/xs representation (per-layer macro Σ_t/Σ_tr per group + the per-isotope collision-sampling data `ref/`'s `LayerData` precomputes). DoD: device(float) ↔ CPU(double) **parity within float tolerance** — the tracker on a battery of known rays (axial / tangential / miss / inside-out), and macro Σ vs `mix()` — plus config-independence. Reuse `src/gpu/buffers.cuh`. Keep the CUDA guard so CI stays at 75. See `spec/05-module-transport.md §6`, `spec/04-module-core.md §4`; DoD in `07-milestones.md`. Then **M4-T2-b**: the event loop (tentative-event → prefix-sum partition → branchless kernels) + deterministic fission bank at the M4-T1 scan slots + T-diff vs `ref/` (G0c) on a fixed-source toy.

## Ready-queue (runnable now)

1. **M4-T2-a** (recommended — NEXT ACTION; **claimed in_progress by session-2026-08-02-c**) — device geometry + material/xs (the static data the GPU transport reads), parity-tested vs the CPU. SPLIT from M4-T2.
2. **M4-T2-b** — event transport (prefix-sum partition + branchless kernels) + deterministic fission bank + T-diff (G0c) on a fixed-source toy; needs M4-T2-a.
...
| M4-T2-a | in_progress | session-2026-08-02-c | 2026-08-02 | M4-T1 | SPLIT from M4-T2 (§8): device geometry (float AnalyticSphereTracker) + device material/xs (per-layer macro Σ_t/Σ_tr + per-isotope collision data). DoD = float↔double parity vs CPU tracker / `mix()` + config-independence |
