...
- **Milestone:** M1 — CPU reference transport + bare-sphere benchmarks (M0 complete; M1-T1..T3 + T4a-1 done). **M4 in progress in parallel (ADR-009); SYNC-M4 run; M4-T1 + M4-T2-a + M4-T2-b done — the GPU event-based transport is live and differentially validated against the CPU oracle (G0c-style, statistical).**
- **VERIFY:** `cmake --preset win-x64 && cmake --build --preset win-x64-rel && ctest --preset win-x64-rel` — the `12 §2` canonical loop; falsifiable and real (configure + compile + link + tests). **On the dev machine (CUDA auto-on): 86 passing = 75 CPU + 11 `gpu.` (on the RTX 4070 Ti SUPER). On a CPU-only build / the CI runners (`NUKESIM_WITH_CUDA` OFF): 75.** Takes ~1 min warm. Per-task probes are **anchored** (`11 §1`): `ctest --preset win-x64-rel -R "^<module>\."` for module ∈ {toolchain, constants, oracle, rng, loaders, geometry, ref, eigen, benchmarks, gpu} — all resolve and are disjoint. `^gpu\.` selects 11 and only registers when `NUKESIM_WITH_CUDA` is ON. Disjoint **and** exhaustive still holds: the anchored counts sum to the total — 6+11+1+10+11+15+7+9+5+11 = 86 (or 75 on a CPU-only build).
- **NEXT ACTION:** Execute **M4-T3** — the GPU eigenvalue path. Wrap M4-T2-b's transport so it drives the backend-agnostic `physics/eigen` solver, **propagating** the deterministic fission bank as the fission-source iteration (bank → next-generation source → power iteration E2a + fixed-8³ entropy E2b, on the GPU). Validate with a differential test: GPU eigen `k` vs `ref/` eigen `k` on a benchmark-like toy, statistical equivalence per G0c (`08 §2`: |k_ref−k_gpu| ≤ 3√(σ²+σ²) and ≤ 100 pcm) + same-backend bit-identity across thread counts. Reuse `src/gpu/{transport,device_problem,geometry,materials}` + the bank. **Note:** the FORMAL G0c gate ("`nukebench diff` executed, report recorded") needs the `nukebench` frontend + `gates.toml` from **M1-T5**, which is behind the blocked M1-T4a-2 (owner cited-data) — so M4-T3 lands the eigen path + a differential test now, recording the formal gate once M1-T5 exists. Also: the per-event **branchless-kernel split** M4-T2-b deferred is the **M4-T4** perf/divergence pass (`02 §4` profile-first). See `spec/05-module-transport.md §2`+`§6`, `spec/08-validation.md §2`; DoD in `07-milestones.md`.

## Ready-queue (runnable now)

1. **M4-T3** (recommended — NEXT ACTION; **claimed in_progress by session-2026-08-02-c**) — GPU eigen path (propagate the M4-T2-b fission bank through the power iteration) + differential validation vs ref; unblocked (M4-T2-b done). The formal `nukebench` G0c gate awaits M1-T5 (blocked).
2. **M4-T4** — GPU perf pass + G4 (incl. the per-event branchless-kernel split deferred by M4-T2-b); needs M4-T3.
...
| M4-T3 | in_progress | session-2026-08-02-c | 2026-08-02 | M1-T3, M4-T2-b | gpu eigen: drive `physics/eigen` via a GpuTransport wrapper, **propagate the M4-T2-b fission bank** (fission-source iteration). Differential vs ref now; FORMAL G0c gate needs `nukebench`+`gates.toml` (M1-T5, behind blocked M1-T4a-2) |
| M4-T4 | todo | — | — | M4-T3 | perf pass + G4; includes the per-event **branchless-kernel split** (warp divergence) deferred by M4-T2-b, and Nsight FP64-fraction ≤ 1% |
