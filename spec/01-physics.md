...
- **E4 (energy → expansion, MAJ-10):** internal energy is a state variable: `dE_int/dt = Ė_dep − P_int·dV/dt`, `P_int = (γ−1)·E_int/V`, γ = 5/3 default (SIM-tagged EOS choice, consistent with C-070). Shell motion: `M·d²R/dt² = 4πR²·(P_drive − P_int)`. With Ė_dep = 0, `E_int + ½MṘ²` is conserved to integrator tolerance. There is NO "binding/explosive threshold" term (deleted).
