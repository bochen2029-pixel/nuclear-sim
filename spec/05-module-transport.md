...
Power iteration E2a; convergence E2b (fixed 8³ mesh, mean-H window W=5, `I_min` floor). **Entropy-window noise is correlation-dominated, not batch-dominated (M1-T3, measured):** the E2b test compares two W=5 means of H, so it can only fire if that difference's statistical noise sits below `eig_h_tol`. Measured on a small test sphere, per-generation σ_H was **0.0249 at batch 2.5e3 and 0.0215 at batch 1e4** — a 4× batch increase reduced it by 16%, nowhere near the 2× that independent sampling would give. The fission source of generation *n+1* is sampled FROM generation *n*, so successive H values are strongly autocorrelated and the noise does **not** fall as 1/√N; this is the same inter-cycle correlation that already forces the batched-means σ for k (MAJ-32). Consequence: **C-901's `eig_h_tol = 1e-3` was not achievable at either batch on that system, and raising the batch is not the fix.** Before G0a is claimed, M1-T5 MUST measure σ_H at the actual gate configuration (C-900, 1e6) on a real benchmark and either confirm 1e-3 is reachable or amend C-901 with the measured value — a convergence criterion that can never fire is a silent `I_min`-only test, i.e. no test at all. A unit test on a small system MUST set `h_tol` from its own measured noise rather than assume the solver is broken. Gate configs from `gates.toml` (C-900); interactive from scenario `[eigen]` (C-900b). Λ via track-length time estimator (`01 §3`). M1-T3 DoD additions: (a) deliberately bad initial source (all at r=0) needs ≥5 more inactive gens than uniform, H non-constant in both cases; (b) Λ decreases monotonically with pit density at fixed geometry, Λ(2ρ)/Λ(ρ) ∈ [0.4, 0.6].
