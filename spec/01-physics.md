...
- **E1c (collision, implicit capture — normative, BLK-05):** the collision is on the transport-corrected Σ_tr, CONSISTENT with the E1a Σ_tr free flight (ADR-021) — sample the isotope ∝ n_i·Σ_tr,i. Bank `⌊ w·ν̄_i·(Σ_f,i/Σ_tr,i)·(1/k_gen) + ξ ⌋` progeny (eigenvalue) or push the same expected count onto the stack (fixed source). Then reduce weight to the transport-reduced scatter share: `w ← w·(Σ_s,i − μ̄_i·Σ_s,i)/Σ_tr,i`. **The neutron is NOT killed at fission.** Colliding on Σ_t while flying on Σ_tr would scale the fission source by Σ_tr/Σ_t and over-suppress k as forward-peaking grows; for μ̄=0 (Σ_tr = Σ_t) this reduces to the analog split `⌊w·ν̄_i·(Σ_f,i/Σ_t,i)/k_gen + ξ⌋`, `w ← w·Σ_s,i/Σ_t,i`. Scatter isotropically-in-lab against the transport-corrected frame (angular approximation; recorded limitation R-4) with group change sampled from `transfer[from][·]`. Analog absorption is NOT used; `sigma_c` never terminates a history directly.
